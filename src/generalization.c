...
/* Maze geometry, the suite, the layout encoding and the step function now
   live in mazesuite.c, shared with the other algorithms evaluated on this
   suite. Everything below is specific to the DQN experiments. */
    ...
    float output[GEN_MAX_INPUT])
{
    ...
    EncodeLayout(maze, state, output);
}
...
/* Packs a shared-environment step into the transition the replay buffer
   stores. The maze view travels with the transition because a procedural
   maze may no longer exist by the time the transition is replayed. */
static GenTransition TakeStep(const GenMazeView *maze, int state, Action action)
{
    GenStepOutcome outcome = StepMaze(maze, state, action);
